<%@page import="com.Algorithm.KMeansDatabase"%>
<%@page import="com.project.DAO.ManagerDao"%>
<%@page import="com.project.Bean.TaskBean"%>
                            ...
                            <div class="block-content collapse in">
                                <div class="span12">
                                   <div class="table-toolbar">
                                    ...
                                    ResultSet rs=smt.executeQuery("select * from task where team_name='"+team_name+"'");
                                    Date date=new Date();
                                    ...
                                    <table cellpadding="0" cellspacing="0" border="0" class="table table-striped table-bordered" id="example2" >
                                        ...
                                        <tbody>
                                         <%while(rs.next())
                                        {
                                        	//Date assign=rs.getDate(6);
                                        	Date dead=rs.getDate(7);
                                        	String status=rs.getString(8);
                                        	//System.out.println("Deadline before="+dead.before(sqlDate));
                                        	...
                                        	else if(completion>70 && completion<=90)
                                        	{
                                        		grade="A";
                                        	}else if(completion==100 && dead.after(sqlDate))
                                        	{
                                        		System.out.println("Deadline before in if="+dead.after(sqlDate));
                                        		grade="A++";
                						...
                						<tr class="odd gradeX">
                    						...
                    						<td><%=rs.getDate(6) %></td>
                							<td><%=rs.getDate(7) %></td>
                    						<td><%=rs.getString(8) +" %"%> <a href="TL_ViewStatus.jsp?id=<%=rs.getInt(1)%>">View</a></td>
                    						<TD><%=grade %></TD>
                    					</tr>
            							<%
            							count++;
            							...
            							bean.setTeam_name(rs.getString(2));
            							bean.setTask_name(rs.getString(5));
            							bean.setMember_name(rs.getString(4));
            							bean.setGrade(grade);
            							
            							ManagerDao dao=new ManagerDao();
            							boolean flag=dao.updateGrade(bean);
            							if(flag==true)
            							{
            								System.out.println("Grade updated Successfully");
            								KMeansDatabase db=new KMeansDatabase();
            								db.KmeansClustering();
            							}
                                        }
            							%>
